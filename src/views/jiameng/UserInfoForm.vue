<template>
  <div class="userInfoForm">
    ...
    <div class="stepToNextBtn" @click="stepToNext">下一步</div>
  ...
  methods: {
    ...
    stepToNext () {
      this.verifyData(() => {
        ...
        this.$store.commit('addFormData', {
          key: 'hasBankCard',
          value: this.hasBankCard
        })
        this.$store.commit('addFormData', {
          key: 'bankCardNo',
          value: this.bankCardNo.trim()
        })
        this.$store.commit('addFormData', {
          key: 'hasJobStatus',
        ...
        this.$http.postForm(this, this.$api.addJoinDriver, {
          'member_apply_driver.d_type': 3,
          carte: this.$store.state.formData.data['hasBankCard'] == 0 ? this.$store.state.formData.data['bankCardNo'] : '',
          carte_img: this.$store.state.formData.data['hasBankCard'] == 0 ? this.$store.state.formData.data['bank_card'] : '',
          'member_apply_driver.phone': this.$store.state.userBaseInfo.phone,
          'member_apply_driver.person_name': this.$store.state.formData.data['userName'],
          'member_apply_driver.person_sex': this.$store.state.formData.data['userSex'] == 0 ? '男' : '女',
          'member_apply_driver.idcar_no': this.$store.state.formData.data['userIdCardNo'],
          'member_apply_driver.idcard_reverse': this.$store.state.formData.data['id_card_b'],
          'member_apply_driver.idcard_positive': this.$store.state.formData.data['id_card_a'],
          'member_apply_driver.driverlicense_positive': this.$store.state.formData.data['drive_card'],
          ...
          'member_apply_driver.qualification_certificate': this.$store.state.formData.data['hasJobStatus'] == 0 ? this.$store.state.formData.data['job_status_img'] : '',
          'member_apply_driver.hand_held_card': this.$store.state.formData.data['id_card_hand'],
          'member_apply_driver.is_help_qualification': this.$store.state.formData.data['needHelpJobStatus'] ? 1 : 0
        }).then(userInfoResult => {
          this.$router.replace('/jiameng/carInfoForm')
        })
      })
    },
    verifyData (callBack) {
      ...
      if (this.hasBankCard == 0 && this.bankCardBase64 == null) { this.$comfun.showToast(this, '银行卡照片不能为空'); return false }
      if (this.hasBankCard == 0 && this.bankCardNo == '') { this.$comfun.showToast(this, '银行卡卡号不能为空'); return false }
      if (this.hasBankCard == 0) {
        this.$http.getFull(this, `https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=${this.bankCardNo.trim()}&cardBinCheck=true`, null, false, false).then(bankResult => {
          if (!bankResult.validated) {
            this.$comfun.showToast(this, '银行卡卡号不正确')
          } else {
