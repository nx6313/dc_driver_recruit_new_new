...
export default {
	install: function (Vue, options) {
		...
		vuexPlugin.on('renderTab', function(callback) {
			...
			let html = `<div id="vuexVConsoleDataWrap" style="padding: 10px 10px 0; font-size: 12px !important;">${vuexDataTreeHtml}</div`
			callback(html)
			bindCopyEvent(vconsole, options.showToast)
		})
...
let getVuexDataTreeHtml = function(vconsole, vuexData) {
	...
	for (let key in vuexData) {
		...
		if (vconsole.tool.isString(vuexData[key]) || vconsole.tool.isBoolean(vuexData[key]) || vconsole.tool.isNumber(vuexData[key])) {
			dataType = `<span class="canCopyThis" style="color: #0D7F25;">${vuexData[key]}</span>`
...
let getObjectNextDataHtml = function(vconsole, data, level) {
	...
	for (let nextKey in data) {
		...
		if (vconsole.tool.isString(data[nextKey]) || vconsole.tool.isBoolean(data[nextKey]) || vconsole.tool.isNumber(data[nextKey])) {
			dataType = `<span class="canCopyThis" style="color: #0D7F25;">${data[nextKey]}</span>`
...
// 绑定复制事件
let bindCopyEvent = (vconsole, toastFn) => {
	let vcCopyDoms = vconsole.$dom.querySelectorAll('.canCopyThis')
	copyTxt(toastFn, vcCopyDoms)
}

// 复制文本
let copyTxt = (toastFn, ref, successTip = '复制成功', failTip = '复制失败') => {
